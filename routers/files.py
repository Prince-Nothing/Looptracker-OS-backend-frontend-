...
async def file_status_generator(request: Request, user_id: int):
    ...
    # initial comment to establish stream (helps some proxies)
...
async def file_status_stream(
    ...
    """
    Subscribe to real-time file processing status updates.
    Accepts token via Authorization header or ?token=/ ?access_token= query param.
    """
    headers = {
        "Cache-Control": "no-store",
        "X-Accel-Buffering": "no",  # Nginx: do not buffer SSE
    }
    return EventSourceResponse(file_status_generator(request, current_user.id), headers=headers)
